...
class HR extends Component
{
    ...
    public function employeeAction(){
        ...
        approvals::create([
            ...
            'process_name' => $this->permission.'Employee',
            'process_description' =>  auth()->user()->name.' has '.$this->permission.' employee  '.  DB::table('employees')
                    ->where('id',session()->get('deleteEmployeeModal'))->value('first_name'),
            'approval_process_description' => 'delete employee',
            'process_code' => 102,
            'process_id' => session()->get('deleteEmployeeModal'),
    ...
    public function editDepartment(){
        ...
        $department_id=session()->get('departmentIdForEditing');

        $prev_department_name=DB::table('departments')->where('id',$department_id)->value('department_name');
           // DB::table('departments')->where('id',$department_id)->update(['department_name' => $this->department_name]);

        // send for the approvals  // yes //
        approvals::create([
            'institution' =>1,
            'process_name' => 'UpdateDepartment',
            'process_description' => "has updated department name from  $prev_department_name to $this->department_name department",
            'approval_process_description' => 'has edited the department name',
            'process_code' => 102,
            'process_id' => $department_id,
            'process_status' => 'PENDING',
            'user_id'  => auth()->user()->id,
            'team_id'  => "",
            'edit_package'=> $this->department_name,
