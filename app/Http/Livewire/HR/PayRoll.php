...
class PayRoll extends livewiredatatable
{
    ...
    public function builder()
    {
        ...
        return Employee::query()->where('employee_status','!=','DELETED');
    }
    ...
    public function columns(): array
    {
        return [
            ...
            Column::callback('benefits',function($benefitId){
                return number_format(DB::table('benefits')->where('id',$benefitId)->value('amount'),2);
            })
                ->label('Benefits'),
            Column::callback('taxes',function($taxId){
                return number_format( DB::table('taxes')->where('id',$taxId)->value('amount'),2);})
                ->label('Taxes'),

            Column::callback('contribution',function($contributionId){
                return number_format(DB::table('Contributions')->where('id',$contributionId)->value('amount'),2);})
            ...
            Column::callback(['salary','taxes','contribution','benefits'],function ($salary,$taxes,$contribution,$benefits){
                $contribution_amount= DB::table('Contributions')->where('id',$contribution)->value('amount');
                $taxes_amount= DB::table('taxes')->where('id',$taxes)->value('amount');
                $benefits_amount= DB::table('benefits')->where('id',$benefits)->value('amount');

                $total=(double)$salary-(double)$taxes_amount-(double)$contribution_amount+(double)$benefits_amount;
