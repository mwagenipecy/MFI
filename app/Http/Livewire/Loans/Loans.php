...
class Loans extends Component
{
    ...
    protected $listeners = [
        ...
        public function mount(){
            Session::put('LoanStage','PROGRESS');
        }
...
//            session::put('currentloanClient',null);
        ...
        if($tabId == '3')
        {
            Session::put('LoanStage','COMMITTEE');
        }

        if($tabId == '4')
        {
            Session::put('LoanStage','ARREARS');
        }
        if($tabId == '5')
        {
            Session::put('LoanStage','RECOVERY');
        }
        if($tabId == '6')
        {
            Session::put('LoanStage','WRITEOFF');
        }
...
//        dd(session::get('loanStageId'));
    ...
    public function issueLoans()
    {
        ...
        $loans_account_new_balance = (double)AccountsModel::where('account_number',$this->linked_loans_account)->value('balance') - ($this->number_of_loans * $this->nominal_price);
        $loan_account_new_balance = (double)AccountsModel::where('account_number',$this->account_number)->value('balance') + ($this->number_of_loans * $this->nominal_price);
        $loan_ledger_account_new_balance = (double)AccountsModel::where('account_number','5111108827')->value('balance') + ($this->number_of_loans * $this->nominal_price);
