...
//        $this->loan_type=session()->get('loan_type');
    ...
    public function set()
    {
        ...
        $accountNumber = str_pad(MembersModel::where('member_number', $this->member_number)->value('branch'), 2, '0', STR_PAD_LEFT) . '104' . str_pad(MembersModel::where('member_number', $this->member_number)->value('id'), 5, '0', STR_PAD_LEFT);
        $loan_id = time();

        $id = AccountsModel::create([
            'account_use' => 'external',
            'institution_number' => '999999',
            'branch_number' => str_pad(MembersModel::where('member_number', $this->member_number)->value('branch'), 2, '0', STR_PAD_LEFT),
            'member_number' => $this->member_number,
            'product_number' => $this->product_number,
            'sub_product_number' => $this->product_number,
            'account_name' => MembersModel::where('membership_number', $this->member_number)->value('first_name') . ' ' . MembersModel::where('member_number', $this->member_number)->value('middle_name') . ' ' . MembersModel::where('member_number', $this->member_number)->value('last_name'),
        ...
        LoansModel::create([
            ...
            'branch_id' => MembersModel::where('member_number', $this->member_number)->value('branch'),
        ]);
