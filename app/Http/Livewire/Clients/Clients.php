...
class Clients extends Component
{
    ...
    protected $rules = [
        ...
        'nationarity' => 'nullable|string|max:255',
    ...
    public function viewClientLoan($member_number){
    ...
    public function updatedMember()
    {
        ...
        $memberData = MembersModel::where('id', $this->client)->get();
        foreach ($memberData as $member) {
            ...
            $this->vehicle_number = $member->vehicle_number;
    ...
    public function updateClient()
    {
        ...
        if (MembersModel::where('id', $this->client)->value('client_status') == "NEW CLIENT") {
            $ClientId=[];
              $last_member = MembersModel::where("client_status",'!=',"NEW CLIENT")->latest()->first();


            if($last_member) {

                $last_member_id = (int)$last_member->member_number;
            ...
            $officer_phone_number=Employee::where('id',MembersModel::where('id',$this->client)->value('loan_officer'))->value('email');
            ...
            MembersModel::where('id',$this->client)->update(['member_number'=>$ClientId,'client_status'=>"ONPROGRESS" ]);

            LoansModel::where('client_id',$this->client)->where('loan_id',$loan_id)->update(['status'=>"ONPROGRESS",'member_number'=>$ClientId ]);
        }
        ...
        MembersModel::where('id', $this->client)->update([
            'updated_at' => now(),
...
//        $this->showAddMember = false;
    ...
    public function editClientModal($id)
    {
        ...
        $this->client = $id;
        $this->pendingClientname = MembersModel::where('id',$id)->value('first_name');
        $this->updatedMember();
    ...
    {
        $user = MembersModel::where('id',$this->clientSelected)->first();
        ...
        if ($user) {
            if($this->permission == 'BLOCKED'){
              ...
              $loan=  LoansModel::where('member_number',$user->member_number)->where('status',"ACTIVE")->exists();
              if($loan || $user->client_status=="BLOCKED"){
                  $action="denied";
              }
            }
            ...
            if($action=="denied"){
                Session::flash('message', 'Process denied, review the client');
                Session::flash('alert-class', 'alert-warning');

            }else{
                $update_value = approvals::create(
