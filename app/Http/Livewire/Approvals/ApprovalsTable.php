...
class ApprovalsTable extends LivewireDatatable
{
    ...
    public function approve($id): void
    {
        ...
        if($approval->process_name =='setOperationalAccount'){
            $this->approveSetOperationalAccount($approval->process_id,$id);
        }

        if($approval->process_name =='addEmployee'){
            $this->approveAddEmployee($approval->process_id,$id);
        }
       if($approval->process_name =='DELETEDEmployee'){
            $this->approveDELETEDEmployee($approval->process_id,$id);
        }

        if($approval->process_name =='UpdateDepartment'){
            $this->approveUdateDepartment($approval->process_id,$id);
        }
...
///////////////////////////////////BRANCH//////////////////////////////////////////
 ...
 //////////////////////////////// TELLERS MANAGEMENT/////////////////////////////////////////////
    ...
    public function reject($id): void
    {
        ...
        if($approval->process_name =='setOperationalAccount'){
            $this->rejectSetOperationalAccount($approval->process_id,$id);
        }

         if($approval->process_name =='addEmployee'){
             $this->rejectAddEmployee($approval->process_id,$id);
            }

        if($approval->process_name =='UpdateDepartment'){
            $this->rejectUpdateDepartment($approval->process_id,$id);
        }
...
//      AccountsModel::where('account_number', $mirror_account)->update(['balance' => $savings_ledger_account_new_balance]);
    ...
    public function approveAddEmployee($process_id,$approvalsId){

        $edit_package=  approvals::where('id', $approvalsId)->value('edit_package');
        $array=json_decode($edit_package);
        // register user
        ...
        DB::table('employees')->where('id',$process_id)->update([
        ...
        Mail::to($array->email)->send(new EmployeeRegisterMail($employeeId,$user_email,$name,$password));
    ...
    public function approveDELETEDEmployee($process_id,$approvalsId){

        // delete users
        DB::table('users')->where('employeeId',$process_id)->update([
          'status'=>'DELETED'
        ]);
    ...
    public function approveSetOperationalAccount($process_id,$approvalsId){
        $package= approvals::where('id', $approvalsId)->value('edit_package');
    ...
    public function approveUdateDepartment($process_id,$approvalsId){

           $approval=DB::table('approvals')->where('id',$approvalsId)->first();

           //edit department
        DB::table('departments')->where('id',$process_id)->update([
            'department_name'=>$approval->edit_package,
        ]);
...
//        Teller::where('id',$process_id)->update(['status'=>'REJECTED']);
    ...
    public function rejectSetOperationalAccount($process_id,$approvalsId):void{
//        Teller::where('id',$process_id)->update(['status'=>'REJECTED']);
        approvals::where('id', $approvalsId)->update([
            'approver_id' => auth()->user()->id,
            'process_status' => 'REJECTED',
            'approval_status' => 'REJECTED',
            'approval_process_description' => 'rejected   ',

        ]);
    }
    public function rejectAddEmployee($process_id,$approvalsId):void{
    ...
    public function rejectUpdateDepartment($process_id,$approvalsId):void{
//        Teller::where('id',$process_id)->update(['status'=>'REJECTED']);
        approvals::where('id', $approvalsId)->update([
            'approver_id' => Auth::user()->id,
            'process_status' => 'REJECTED',
...
///////////////////////////////////END OF CHANNEL////////////////////////////////////////
    ...
    public function approveDeleteUser($process_id,$approvalsId): void
    {
        ...
        $get_suggested_status=approvals::where('id',$approvalsId)-> first();
        

        DB::table('clients')->where('id',$process_id)->update([
            'client_status'=>$get_suggested_status->process_status,
        ]);


        $client_number=DB::table('clients')->where('id',$process_id)->value('client_number');
        if($client_number){
            if(   DB::table('loans')->where('client_number',$client_number)
                ->where('status','ONPROGRESS')->update([
                    'status'=>'NEW CLIENT'
                ])){
                ...
                DB::table('loans')->where('client_number',$client_number)
                    ->where('status','NEW CLIENT')->update([
                        'status'=>'ONPROGRESS'
                    ]);
            }
        }
