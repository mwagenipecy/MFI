...
class FrontDesk extends Component
{
        ...
        if ($this->payment_method == "CASH") {
            ...
            if (AccountsModel::where('id', Teller::where('employee_id', auth()->user()->employeeId)->value('account_id'))->exists()) {
                ...
                if ($teller_balance < $this->amount3) {
                    ...
                    // update balance
                    $teller_new_balance = (float)$teller_balance - (float)$this->amount3;
            ...
            $mirror_account_new_balance = (float)$mirror_account_array_data->balance + (float)$this->amount3;
            ...
            $general_ledger_records->credit($mirror_account_array_data->account_number, $mirror_account_new_balance, $custome_loan_account_data->loan_account_number, (float)$this->amount3, 'bank/ mobile transaction', '');
            ...
            $customer_new_balance = (float)$cutomer_account_data->balance - (float)$this->amount3;
            ...
            $bank_new_balance = (float)$mirror_account_data->balance + (float)$this->amount3;
    ...
    public function process()
    {
        ...
        // credit
            ...
            $new_teller_balance = (float)$this->amount + (float)$teller_account_data->balance;
            // update
            ...
            $mirrorr_account_new_balance = (float)$bank_balance - (float) $this->amount;
            ...
            //update balance
            $member_savings_account_new_balance = (float)$member_savings_account_details->balance - (float)$this->amount;
            // debit on mirror account
            ...
            $new_customer_balance = (float)$customer_account_data->balance - (float)$this->amount;
            // update
            AccountsModel::where('account_number', $customer_account_data->account_number)->update(['balance' => $new_customer_balance]);
    ...
    function update_repayment($loan_id, $amount)
    {
        ...
        foreach ($schedules as $schedule) {
            ...
            // Determine the completion status
            if ((float)$total_payment >= (float)$schedule->installment) {
                $completion_status = 'PAID';
            } else {
                $completion_status = 'PARTIAL';
            }
